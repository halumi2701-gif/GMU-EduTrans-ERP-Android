...
function applyV21Branding(source) {
  ...
  output = output.replace(
    ...
    'GMU EduTrans Company Autopilot v21.3 aktif • Seluruh fitur lama tetap dipertahankan; target, Sales, Booking, Operasional, Finance, Payment Request, Payroll/Fee, Recruitment dan Workforce terhubung end-to-end.'
  );
  return output;
}
...
async function main() {
  ...
  const companyControlSource = fs.readFileSync(path.join('erp-web', 'company-control-v213.js'), 'utf8');
  assertIncludes(loaderSource, 'v21.3-company-control-center', 'v21.3 loader marker');
  ...
  assertIncludes(loaderSource, 'company-control-v213.js', 'v21.3 company control module');
  assertIncludes(loaderSource, 'v21.3-company-control-center-layer', 'v21.3 release layer');
  ...
  assertIncludes(companyControlSource, "const VERSION = 'v21.3-company-control-center'", 'company control UI version');
  assertIncludes(companyControlSource, 'v_company_command_center', 'company command center view');
  assertIncludes(companyControlSource, 'payment_requests', 'payment request integration');
  assertIncludes(companyControlSource, 'compensation_accruals', 'compensation integration');
  assertIncludes(companyControlSource, 'recruitment_cases', 'recruitment integration');

  fs.writeFileSync(path.join(OUT_DIR, 'vercel.json'), JSON.stringify({
    cleanUrls: true,
    trailingSlash: false,
    headers: [{ source: '/(.*)', headers: [
      { key: 'X-GMU-ERP-Release', value: 'v21.3-company-control-center' },
      ...
      { key: 'X-GMU-ERP-Company-Control-Layer', value: 'v21.3-company-control-center-layer' },
      { key: 'Cache-Control', value: 'no-store, max-age=0' },
    ] }],
  }, null, 2) + '\n', 'utf8');

  const manifest = {
    release: 'GMU EduTrans Company Control Center v21.3',
    strategy: 'additive-company-control-on-existing-autopilot-and-verified-pinned-v9.5-baseline',
    ...
    enterpriseFlow: ['Director Target','CRM/Sales','Quotation','Payment','Booking','Operations','Crew/Vendor','Trip','Finance','Payroll/Fee','Recruitment/Workforce','Feedback','Repeat Order'],
    companyControlCapabilities: ['Rp50m monthly revenue target','25% margin floor','3x pipeline coverage','Payment Request/AP control','Compensation lifecycle','Recruitment pipeline','Workforce guardrail','Owner/Manager/Finance command center'],
