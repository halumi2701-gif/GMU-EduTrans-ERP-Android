...
function applyV109Branding(source) {
  let output = source.replaceAll('v9.5', 'v10.9');
  output = output.replace('ERP v10.9 — Online Multi-User','ERP v10.9 — Company Operating System');
  output = output.replace(
    'v10.9 memakai database online dengan Trip Folder, private document upload, approval, Trip Archive, Financial Privacy Guard, dan dukungan 3 bahasa. Perubahan antar user akan disinkronkan ulang melalui Supabase Realtime.',
    'ERP Web v10.9 aktif • Full Company Operating System: Jobdesk & SOP per jabatan, Tugas Otomatis, Penggajian & Fee, Rekrutmen & Onboarding, Keluhan & CAPA, Perkiraan Kas, Perencanaan SDM, Daftar Risiko, Target & Kinerja, Intelijen Pasar Cianjur–Sukabumi, Kas & Likuiditas, SDM, Mutu, Pertumbuhan, Tata Kelola, Pusat AI, Trip Folder, Approval, dan Supabase Realtime.'
  );
  return output;
}
...
async function main() {
  const response = await fetch(VERIFIED_BASELINE_URL, {
    headers: { accept: 'text/html,application/xhtml+xml', 'user-agent': 'GMU-EduTrans-ERP-v10-release-builder/3.3' },
    redirect: 'follow',
  });
  ...
  const brandedBaseline = applyV109Branding(baseline);
  assertIncludes(brandedBaseline, '<title>GMU EduTrans ERP v10.9', 'v10.9 browser title');
  assertIncludes(brandedBaseline, 'ERP v10.9 — Company Operating System', 'v10.9 login branding');
  assertIncludes(brandedBaseline, 'ERP v10.9 Online', 'v10.9 sidebar branding');
  ...
  assertIncludes(brandedBaseline, 'Penggajian & Fee', 'payroll visible notice');
  assertIncludes(brandedBaseline, 'Rekrutmen & Onboarding', 'recruitment visible notice');
  assertIncludes(brandedBaseline, 'Keluhan & CAPA', 'quality visible notice');
  assertIncludes(brandedBaseline, 'Perkiraan Kas', 'cash forecast visible notice');
  assertIncludes(brandedBaseline, 'Perencanaan SDM', 'workforce visible notice');
  assertIncludes(brandedBaseline, 'Daftar Risiko', 'risk visible notice');
  ...
  fs.writeFileSync(path.join(OUT_DIR, 'vercel.json'), JSON.stringify({
    ...
    headers: [{ source: '/(.*)', headers: [
      { key: 'X-GMU-ERP-Release', value: 'v10.9-execution-control-system' },
      { key: 'Cache-Control', value: 'no-store, max-age=0' },
    ] }],
  }, null, 2) + '\n', 'utf8');

  const manifest = {
    release: 'GMU EduTrans ERP Web v10.9 — Execution & Control System',
    ...
    modules: MODULES,
    visibleBranding: 'v10.9',
    systemModules: ['Sistem Perusahaan','Workspace per-role','Tugas Saya','Target & Kinerja','Intelijen Pasar','Kas & Likuiditas','SDM & Personalia','Mutu & Pelanggan','Pertumbuhan','Tata Kelola','Pusat AI','Role Navigation Guard','Detail Control Layer','Jobdesk & SOP per Jabatan','Tugas Otomatis','Penggajian & Fee','Rekrutmen & Onboarding','Keluhan & CAPA','Perkiraan Kas','Perencanaan SDM','Daftar Risiko'],
  ...
  console.log(`ERP Web v10.9 execution-control release built: ${OUT_DIR}`);
  console.log(`Verified v9.5 rollback baseline preserved: ${manifest.baselineBytes} bytes`);
  console.log('Visible production branding: v10.9');
