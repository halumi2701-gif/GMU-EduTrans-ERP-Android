...
private class SalesCommercialApiV226 {
    ...
    private fun request(method: String, path: String, body: String?, accessToken: String): String {
        ...
        val response = BufferedReader(
            InputStreamReader(stream ?: throw IllegalStateException("Response kosong"))
        ).use { it.readText() }
        conn.disconnect()
        if (code !in 200..299) {
            val message = runCatching {
                val root = JSONObject(response)
                root.optString("message", root.optString("error", response))
...
fun SalesCommercialBookingV226Screen(
    ...
    Column(Modifier.fillMaxSize().padding(horizontal = 16.dp)) {
        ...
        Card(shape = RoundedCornerShape(18.dp), colors = CardDefaults.cardColors(containerColor = GmuSoft)) {
            Column(Modifier.padding(14.dp)) {
                ...
                Text("B2B: harga sekolah Rp49.500 • cashback Rp2.500/pax • komisi Sales Rp5.000/pax", fontSize = 11.sp, color = GmuGreen)
            }
        }
    ...
    if (add) {
        SalesCommercialBookingDialogV226(
            ...
            onCreated = { draft ->
                if (vm.actionBusy) return@SalesCommercialBookingDialogV226
                vm.actionBusy = true
                screenScope.launch {
                    try {
...
private fun SalesCommercialBookingDialogV226(
    ...
    LaunchedEffect(Unit) {
        ...
        try {
            ...
            selectedPackage = packages.firstOrNull { it.packageCode == "STATION-PROF-2026" }
    ...
    LaunchedEffect(selectedPackage?.packageCode, channel, sessionType, partner?.id, ownPax, pricingPax, date) {
        ...
        if (channel == "B2B_MOU" && partner == null) {
            previewError = if (partners.isEmpty()) {
                "Belum ada Partner B2B dengan MoU/PKS aktif."
            } else {
                "Pilih Partner B2B aktif."
            }
    ...
    AlertDialog(
                        ...
                        packages.forEach { p ->
                            DropdownMenuItem(
                                text = {
                                    Column {
                                        Text(p.programName)
                                        Text(p.packageName, fontSize = 11.sp, color = Color.Gray)
                                    }
                                },
                ...
                if (channel == "B2B_MOU") {
                    ...
                    Box {
                        ...
                        DropdownMenu(partnerMenu, onDismissRequest = { partnerMenu = false }) {
                            if (partners.isEmpty()) {
                                DropdownMenuItem(
                                    text = { Text("Belum ada Partner aktif") },
                                    onClick = { partnerMenu = false }
                                )
                            }
                            partners.forEach { p ->
                                DropdownMenuItem(
                                    text = {
                                        Column {
                                            Text(p.name)
                                            Text(
                                                listOf(p.code, p.agreementNo).filter { it.isNotBlank() }.joinToString(" • "),
                                                fontSize = 10.sp,
                                                color = Color.Gray
                                            )
                                        }
        ...
        confirmButton = {
            ...
            val canSave = !busy && !previewBusy && customer != null && selectedPackage != null &&
                ownPax > 0 && pricingPax > 0 && validDate && resolved != null &&
                resolved.eligible && resolved.status == "SAFE"
