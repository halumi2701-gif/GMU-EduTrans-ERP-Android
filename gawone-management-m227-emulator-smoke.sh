...
APK="GAWONE-Management-v1.0.10-M2.27-WORKFORCE-LIVE-debug.apk"
REPORT_DIR="management-m227-emulator-reports"
mkdir -p "$REPORT_DIR"
...
resolve_launcher() {
  adb shell cmd package resolve-activity --brief -a android.intent.action.MAIN -c android.intent.category.LAUNCHER "$PKG" | tr -d '\r' | tail -n 1
}

launch_app() {
  local label="$1"; local component
  component="$(resolve_launcher)"
  echo "$component" | tee "$REPORT_DIR/${label}-launcher.txt"
  test -n "$component"; test "$component" != "No activity found"
...
launch_app "03-cold"; check_process "04-cold"
wait_ui_text "Command Center GAWONE" "05-cold-ui"
wait_ui_text "Masuk Management" "06-login-gate"
check_no_crash "07-cold"
...
adb shell am force-stop "$PKG"; adb logcat -c
launch_app "08-offline"; check_process "09-offline"
...
sleep 4
adb shell am force-stop "$PKG"; adb logcat -c
launch_app "12-reconnect"; check_process "13-reconnect"
...
echo "GAWONE Management M2.27 Workforce Live Operations emulator gate: PASS" | tee "$REPORT_DIR/RESULT.txt"
