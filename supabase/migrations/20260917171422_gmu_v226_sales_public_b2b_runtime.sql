-- GMU EduTrans v22.6 — Sales PUBLIC + B2B runtime
-- Production-aligned tier master, booking snapshots, safe views and server-side pricing/booking RPCs.
...
begin
  ...
  where id=v_pkg.program_id
  limit 1;

  if p_booking_pax<v_pkg.min_pax then
    return jsonb_build_object('eligible',false,'status','BLOCKED','reason','MIN_BOOKING_PAX_NOT_MET','min_pax',v_pkg.min_pax);
  end if;
  ...
  if p_package_code='STATION-PROF-2026' then
    if v_channel='DIRECT_PUBLIC' then
      ...
      if v_session='PRIVATE' then
        ...
        return jsonb_build_object('eligible',false,'status','BLOCKED','reason','SHARED_SESSION_MIN_40','required_pricing_pax',40);
      end if;
    ...
    v_margin:=case
      when v_gmu_net*v_pricing_pax>0
...
begin
  ...
  v_price:=public.resolve_sales_price_v226(
    p_package_code,p_booking_pax,upper(p_channel),p_partner_id,
    upper(p_session_type),p_pricing_pax,p_trip_date
  );
  ...
  if v_status='REVIEW'
     and coalesce(v_role,'') not in ('Owner','Director','Direktur','Manager','Manager EduTrans') then
  ...
  insert into public.bookings(
    ...
    p_package_code,
    upper(p_channel),
    upper(p_session_type),
    coalesce(p_pricing_pax,p_booking_pax),
    p_partner_id,
    v_customer_price,
    v_cashback,
    v_partner_margin,
