-- GMU EduTrans ERP v20 — Company-wide control, finance, compensation and HR
-- Safe additive migration. Existing operational tables remain authoritative.

create extension if not exists pgcrypto;

-- 1) Company targets and guardrails
...
-- 2) Monthly target and scorecard by role/person
create table if not exists public.performance_targets (
  ...
  updated_at timestamptz not null default now(),
  unique(period_month, role_name, staff_id)
);
...
-- 3) Compensation policy and automatic fee lifecycle
...
values
  ('SALES_DEFAULT','Sales','SALES_COMMISSION','COLLECTED_REVENUE_PCT',3,0,99.99,1,25,'Default jika master paket tidak memiliki fee khusus'),
  ...
  ('MANAGER_BONUS_100','Manager EduTrans','TARGET_BONUS','COLLECTED_REVENUE_PCT',0.50,100,119.99,1,25,'Bonus manager jika target perusahaan tercapai'),
  ('MANAGER_BONUS_120','Manager EduTrans','TARGET_BONUS','COLLECTED_REVENUE_PCT',0.75,120,null,1,25,'Bonus manager jika pencapaian >=120%'),
  ('TEAM_POOL_100','Support','TEAM_BONUS','TEAM_POOL_PCT',1.00,100,119.99,1,25,'Pool tim pada 100-119% target'),
  ('TEAM_POOL_120','Support','TEAM_BONUS','TEAM_POOL_PCT',1.50,120,null,1,25,'Pool tim pada >=120% target')
on conflict (policy_code) do update set
  rate=excluded.rate, threshold_min_pct=excluded.threshold_min_pct, threshold_max_pct=excluded.threshold_max_pct,
  multiplier=excluded.multiplier, margin_floor_pct=excluded.margin_floor_pct, active=true, updated_at=now();
...
-- 4) Accounts payable / payment request control for every outgoing payment
...
-- 5) Recruitment pipeline detail layered on top of recruitment_cases
...
create policy company_control_settings_read on public.company_control_settings for select to authenticated using (
  exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true)
);
create policy company_control_settings_write on public.company_control_settings for all to authenticated using (
  exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true and p.role::text in ('Owner','Director'))
) with check (
  exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true and p.role::text in ('Owner','Director'))
);

create policy performance_targets_read on public.performance_targets for select to authenticated using (
...
create policy performance_targets_manage on public.performance_targets for all to authenticated using (
...
create policy performance_snapshots_read on public.performance_snapshots for select to authenticated using (
  staff_id=(select auth.uid()) or exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true and p.role::text in ('Owner','Director','Manager','Manager EduTrans'))
);
create policy performance_snapshots_manage on public.performance_snapshots for all to authenticated using (
  exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true and p.role::text in ('Owner','Director','Manager','Manager EduTrans'))
) with check (
  exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true and p.role::text in ('Owner','Director','Manager','Manager EduTrans'))
);

create policy compensation_policies_read on public.compensation_policies for select to authenticated using (
  exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true)
);
create policy compensation_policies_write on public.compensation_policies for all to authenticated using (
  exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true and p.role::text in ('Owner','Director'))
) with check (
  exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true and p.role::text in ('Owner','Director'))
);

create policy compensation_accruals_read on public.compensation_accruals for select to authenticated using (
...
create policy compensation_accruals_manage on public.compensation_accruals for all to authenticated using (
...
create policy payment_requests_create on public.payment_requests for insert to authenticated with check (
  requested_by=(select auth.uid()) or exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true and p.role::text in ('Owner','Director','Manager','Manager EduTrans','Finance','Admin','Operation'))
);
create policy payment_requests_manage on public.payment_requests for update to authenticated using (
  exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true and p.role::text in ('Owner','Director','Manager','Manager EduTrans','Finance'))
) with check (
  exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true and p.role::text in ('Owner','Director','Manager','Manager EduTrans','Finance'))
);

create policy recruitment_candidates_manage on public.recruitment_candidates for all to authenticated using (
  exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true and p.role::text in ('Owner','Director','Manager','Manager EduTrans'))
) with check (
  exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true and p.role::text in ('Owner','Director','Manager','Manager EduTrans'))
);
create policy recruitment_gate_reviews_manage on public.recruitment_gate_reviews for all to authenticated using (
  exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true and p.role::text in ('Owner','Director','Manager','Manager EduTrans'))
) with check (
  exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.is_active=true and p.role::text in ('Owner','Director','Manager','Manager EduTrans'))
);

-- 7) Owner/Manager command view. security_invoker preserves RLS of underlying tables.
